...
META_DROP = ["serial_id", "cycle_number", "cycle_timestamp", "good_label", "split"]
...
def main():
    ...
    df = pd.read_parquet("outputs/features_spark.parquet")
    print("Columns:", df.columns.tolist())
    candidates = [c for c in df.columns if "serial" in c.lower() or "id" in c.lower()]
    print("ID-like columns:", candidates)
    print(df[candidates[:1]].head() if candidates else "No obvious ID column guessed.")
    print("Rows total:", len(df))
    print("Has columns:", set(["split","CycleTime_sec"]).issubset(df.columns))
    print(df["split"].value_counts(dropna=False))
    print("Target non-null per split:")
    print(df.groupby("split")["CycleTime_sec"].apply(lambda s: s.notna().sum()))
    ...
    if "split" in df.columns:
        ...
        Xtr = train_df.drop(columns=[c for c in META_DROP + [TARGET] if c in train_df.columns])
        ytr = train_df[TARGET].values

        #Xva, yva = val_df.drop(columns=[TARGET, "split"]),   val_df[TARGET].values
        Xva = val_df.drop(columns=[c for c in META_DROP + [TARGET] if c in val_df.columns])
        yva = val_df[TARGET].values

        #Xte, yte = test_df.drop(columns=[TARGET, "split"]),  test_df[TARGET].values
        Xte = test_df.drop(columns=[c for c in META_DROP + [TARGET] if c in test_df.columns])
        ...
        X = df.drop(columns=[c for c in META_DROP + [TARGET] if c in df.columns])
        y = df[TARGET].values
    ...
    assert "split" not in num_cols, f"'split' leaked into features: {num_cols[:10]}..."
    from sklearn.compose import ColumnTransformer
    from sklearn.preprocessing import StandardScaler
    pre = ColumnTransformer([("scale", StandardScaler(), num_cols)], remainder="drop")
