...
def main():
    ...
    # ship utils.py to executors and import it
    ...
    # Robust timestamp parsing — try multiple formats (US-style month/day)
    formats = [
        "M/d/yyyy H:mm",       # e.g., 7/8/2025 9:05
        "M/d/yyyy HH:mm",      # e.g., 7/8/2025 09:05
        "M/d/yyyy H:mm:ss",    # with seconds
        "M/d/yyyy HH:mm:ss",
        "yyyy-MM-dd HH:mm:ss", # ISO-ish fallback
        "yyyy-MM-dd'T'HH:mm:ss"
    ]

    parse_exprs = [F.to_timestamp(F.col(ts_time), fmt) for fmt in formats]
    df = df.withColumn(ts_time, F.coalesce(*parse_exprs))

    df_nonnull = df.filter(F.col(ts_time).isNotNull())

    # If everything is null, bail with a helpful message
    if df_nonnull.rdd.isEmpty():
        raise ValueError(
            f"No timestamps could be parsed in column '{ts_time}'. "
            f"Edit timestamp formats in etl_spark.py (or add to config.yaml) to match your data, e.g. 'M/d/yyyy H:mm'."
        )
    ...
    order_expr = F.when(
        ...
        F.col(ts_time).cast("long")
    )


    #w = Window.partitionBy(id_col).orderBy(order_expr)

    # Normalize id_col from config: treat "", "none", "null" as unset
    id_name = cfg.get("id_column")
    if id_name is None or str(id_name).strip().lower() in ("", "none", "null"):
        # Single stream: global ordering only (NO partition)
        w = Window.orderBy(order_expr)
    else:
        # Multi-stream: partition by the true stream id
